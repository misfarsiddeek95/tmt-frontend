(function () {
  ...
  function liveSearch() {
    ...
    let data = [];
    $.get(`${BASE_URL}get-search-data`, function (response) {
    ...
    const showList = (searchTerm, resultsEl) => {
      ...
      data
        ...
        .forEach((e) => {
          ...
          console.log("list", e);
          div.innerHTML = `
          <button class="-link d-block col-12 text-left rounded-4 px-20 py-15 js-search-option dropdown-button" type="${e.type}" value="${e.id}" name-value="${e.name}" icon="${e.icon}" country="${e.country}">
            <div class="d-flex">
              <div class="${e.icon} text-light-1 text-20 pt-4"></div>
              <div class="ml-10">
    ...
    targets.forEach((item) => {
      ...
      counters.forEach((el) => {
        ...
        buttonDown.addEventListener("click", () => {
          const adultsElement = buttonDown.closest(".adults");
          const roomElement = buttonDown.closest(".rooms");
          if (count.innerHTML != 0) {
            if (
              (adultsElement || roomElement) &&
              parseInt(count.innerHTML) - 1 == 0
            ) {
              count.innerHTML = 1;
            } else {
    ...
    targets.forEach((el) => {
      ...
      noUiSlider.create(slider, {
        ...
        format: {
          ...
          from: function (value) {
            return Math.round(value.replace(/\$/g, ""));  // Remove the $ sign and round the value
          },
        },
      });
      ...
      slider.noUiSlider.on("update", function (values, handle) {
        snapValues[handle].innerHTML = values[handle];
        const existVal = $(`#price${handle}`).val(); // get the existing value
        if (existVal != values[handle].replace(/\$/g, "")) {
          $(`#price${handle}`)
            .val(values[handle].replace(/\$/g, ""))
            .trigger("change"); // set from and to prices to the hidden inputs in the list page when old value and new values are different.
        }
      });
    });
  }
  ...
  function calendarInteraction() {
    ...
    target.forEach((elTarget) => {
      const gridCells = elTarget.querySelectorAll(
        ".table-calendar__grid > *:not(.date-disabled)"
      ); // only select the dates which doesn't have date-disabled class.
      ...
      gridCells.forEach((el, i) => {
        el.addEventListener("click", () => {
          ...
          const oldEndDate = localStorage.getItem('searchEndDate');
          ...
          if (completeState) {
            ...
            // set start and end date to the local storage.
            localStorage.setItem(
              "searchStartDate",
              firstItem.getAttribute("date")
            );
            if ($("#input_start_date").length) {
              $("#input_start_date").val(firstItem.getAttribute("date"));
            }

            localStorage.setItem(
              "searchEndDate",
              lastItem.getAttribute("date")
            );
            if ($("#input_end_date").length) {
              $("#input_end_date").val(lastItem.getAttribute("date"));
            }

            completeState = true;

            const newEndDate = localStorage.getItem('searchEndDate');

            // close the calendar after selecting the end date.
            if(completeState && oldEndDate !== newEndDate) {
              $('.js-form-dd.js-calendar').removeClass('-is-dd-wrap-active');
              $('.searchMenu-date__field').removeClass('-is-active');
            }
          }
        });
      });
